...
WorldResourceLoader::WorldResourceLoader(GameDataHolder* dataHolder) {
    ...
    auto functor = al::FunctorV0M<WorldResourceLoader*, void (WorldResourceLoader::*)()>(this, &WorldResourceLoader::loadResource);

    mWorldResourceLoader = new al::AsyncFunctorThread("WorldResourceLoader",
                                                      functor, priority, 0x100000, sead::CoreId::cMain);
}
...
bool WorldResourceLoader::requestLoadWorldHomeStageResource(s32 loadWorldId, s32 scenario) {
    ...
    if(loadWorldId || scenario != 1) {
        ...
        mCapWorldHeap = sead::FrameHeap::create(0x1F400000, "ワールド常駐[帽子]", mWorldResourceHeap, 8, sead::Heap::HeapDirection::cHeapDirection_Forward, false);
        al::addNamedHeap(mCapWorldHeap, "WorldResourceCap");

        mWaterfallWorldHeap = sead::FrameHeap::create(0, "ワールド常駐[滝]", mWorldResourceHeap, 8, sead::Heap::HeapDirection::cHeapDirection_Reverse, false);
...
al::Resource *WorldResourceLoader::tryLoadResource(char const* resPath, char const* category, char const* str3) {
    if(category) {
        if(str3) {
...
void WorldResourceLoader::loadWorldResource(int loadWorldId, int scenario, bool isScenarioResources, char const* resourceCategory) {
    nn::os::GetSystemTick();
    nn::os::GetSystemTick();
    ...
    if(isScenarioResources) {
        if(!al::tryGetByamlIterByKey(&resourceListIter, loadWorldIter, al::StringTmp<32>("Scenario%d", scenario).cstr()))
            return;
    }else {
...
float WorldResourceLoader::calcWorldResourceHeapSize(void) const {
    return ((mWorldResourceHeap->getSize() - mWorldResourceHeap->getFreeSize()) * 0.00097656f) * 0.00097656f;
}