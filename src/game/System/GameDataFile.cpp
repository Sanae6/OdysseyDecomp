...
bool GameDataFile::HintInfo::isEnableUnlock(int curWorldId, bool isGameClear, int scenarioNo, bool isInWorld) const {
...
bool GameDataFile::HintInfo::isHintStatusUnlock(int curWorldId, int scenarioNo, bool isInWorld) const {
    if(mWorldIndex != curWorldId || unkBool1 || mHintStatus != HintStatus::NONE || (isInWorld ? !mIsMoonRock : mIsMoonRock))
        return false;
...
bool GameDataFile::HintInfo::isEnableNameUnlockByScenario(int curWorldId, int scenarioNo, bool isInWorld) const {
...
bool GameDataFile::HintInfo::testFunc(int curWorldId, bool isGameClear, int scenarioNo, bool isInWorld) const {
    if(isDisableByWorldWarpHole(isInWorld)) {
        return false;
    }
